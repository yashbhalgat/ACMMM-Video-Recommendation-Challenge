...
from tripletnetwork import Tripletnet, SimpleNet
import numpy as np
import pickle
import random
import csv
...
def main():
	...
	if generate_triplets:
		...
		train_size = len(movies_train_gt)
		...
		for iteration in range(33200):
			try:
				...
				n_rel = len(movies_train_gt[p_ind])-1
				if n_rel < 2:
					continue
				...
				p_vid = int(movies_train_gt[p_ind][0])
				pos_vid = int(movies_train_gt[p_ind][pos_ind])
				neg_vid = int(movies_train_gt[p_ind][neg_ind])
		...
		for num_times in range(8):
			for t in range(len(movies_train_gt)):
				try:
					rel_vids = [int(r) for r in movies_train_gt[t]]
					if len(rel_vids) < 2:
						continue
					...
					for sub_iter in range(8):
						...
						for sub_sub in range(8):
							neg_vid = state.randint(len(movies_set))
	...
	shows_set = []
	movies_set = [str(i) for i in range(10826)]
	_, movies_features = get_features(shows_set, movies_set, shows_dir, movies_dir, feature_type="concat", reload_features=False)

	params = get_norm_params(movies_features)
	movies_features = normalize_features(movies_features, params)
	...
	for epoch in range(1, args.epochs + 1):
		# train for one epoch
		train(triplets, movies_features, tnet, criterion, optimizer, epoch, margin)
...
def train(triplets, features, tnet, criterion, optimizer, epoch, margin):
	losses = AverageMeter()
	accs = AverageMeter()
	...
	for batch_idx, (p, p_plus, p_minus) in enumerate(triplets):
		data1 = torch.from_numpy(features[p]).float()
		data2 = torch.from_numpy(features[p_plus]).float()
		data3 = torch.from_numpy(features[p_minus]).float()
		...
		loss = loss_triplet + 0.01 * loss_embedd
		# loss_embedd = 0
...
def accuracy(dista, distb, margin, factor):
	# margin = 0
